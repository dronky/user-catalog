...
import { SystemService } from 'app/entities/system';
...
export class RacfUserUpdateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.systemService
      .query()
      .pipe(
        filter((mayBeOk: HttpResponse<ISystem[]>) => mayBeOk.ok),
        map((response: HttpResponse<ISystem[]>) => response.body)
      )
      .subscribe((res: ISystem[]) => (this.systems = res), (res: HttpErrorResponse) => this.onError(res.message));
  }
  ...
  trackSystemById(index: number, item: ISystem) {
    return item.id;
  }
