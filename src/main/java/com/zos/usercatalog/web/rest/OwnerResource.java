...
/**
 * REST controller for managing {@link com.zos.usercatalog.domain.Owner}.
 */
@RestController
@RequestMapping("/api")
public class OwnerResource {
     ...
     */
    @GetMapping("/owners")
    public ResponseEntity<List<OwnerDTO>> getAllOwners(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder, @RequestParam(required = false) String filter) {
        if ("racfuser-is-null".equals(filter)) {
            log.debug("REST request to get all Owners where racfUser is null");
            return new ResponseEntity<>(ownerService.findAllWhereRacfUserIsNull(),
                    HttpStatus.OK);
        }
    ...
    public ResponseEntity<List<OwnerDTO>> searchOwners(@RequestParam String query, Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
